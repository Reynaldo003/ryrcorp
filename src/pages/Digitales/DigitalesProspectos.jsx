...
function getMesFechaBDC(value) {
    ...
    /*
     * Con USE_TZ = false esperamos valores como:
     *
     * 2026-07-31T18:30:00
     * 2026-08-01T09:00:00
     *
     * Para determinar el mes comercial NO usamos new Date().
     * Tomamos directamente YYYY-MM-DD del backend.
     */
...
function esGestionableBDC(row) {
    return (normalizeText(row?.estado) !== "descalificado" &&
        /^52\d{10}$/.test(normalizaTelefonoMx(row?.telefono)));
}
function esContactadoBDC(row) {
    const estado = normalizeText(row?.estado);
    const estadosConContacto = [
        "contactado",
        "calificado",
    ...
    return Boolean(row?.primer_contacto_at ||
        row?.fecha_contacto ||
        estadosConContacto.includes(estado) ||
        row?.ultima_cita_agendada ||
        row?.asistencia ||
        row?.folio_solicitud_credito ||
        row?.solicitud_credito_estado ||
        row?.vin_facturado);
}
function tieneSolicitudBDC(row) {
    return Boolean(String(row?.folio_solicitud_credito || "").trim() ||
        String(row?.solicitud_credito_estado || "").trim());
}
function esAnfBDC(row) {
    const estado = normalizeText(row?.solicitud_credito_estado);
    return (["autorizado", "condicionado"].includes(estado) &&
        !String(row?.vin_facturado || "").trim());
}
...
function getTipoUnidadBDC(row) {
    ...
    if (agencia.includes("usados"))
        return "Seminuevos";
    if (linea === "usados" || linea === "seminuevos")
        return "Seminuevos";
...
// Citas y Prospectos no siempre guardan el nombre con la misma acentuación.
...
function getEstadoMetaBDC(valor, meta) {
    ...
    if (valor >= meta * 0.6)
        return { label: "Requiere atención", text: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200" };
...
export default function DigitalesProspectos() {
    ...
    const FilterButtonGroup = ({ label, value, options, onChange }) => (<div className="min-w-0">
        <div className="mb-1.5 text-xs font-black uppercase tracking-wide text-[#131E5C]/70">{label}</div>
        <div className="flex gap-2 overflow-x-auto pb-1">
            {options.map((option) => {
                const active = value === option;
                return (<button key={option} type="button" onClick={() => onChange(option)} className={cls("inline-flex h-10 shrink-0 items-center justify-center rounded-lg border px-4 text-sm font-black transition active:scale-[0.98]", active ? "border-[#131E5C] bg-[#131E5C] text-white" : "border-[#131E5C] bg-white text-[#131E5C] hover:bg-[#131E5C]/5")}>
    ...
    return (<div className="w-full">
                ...
                <div className="p-3">
                    <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
                        ...
                        <div className="relative min-w-0 flex-1 mt-5">
                            ...
                            {filters.q ? (<button type="button" onClick={() => updateFilter("q", "")} aria-label="Limpiar búsqueda" className="absolute right-2 top-1/2 inline-flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg text-slate-400 transition hover:bg-red-50 hover:text-red-500">
                                <X className="h-4 w-4" />
                            </button>) : null}
                        </div>
                        {/* Filtros principales */}
                        <div className="grid min-w-0 flex-1 gap-3 xl:grid-cols-[1fr_1fr_180px] xl:items-end">
                            <div>
                                ...
                                <select value={filters.buro} onChange={(e) => updateFilter("buro", e.target.value)} className="h-10 w-full rounded-xl border border-[#131E5C]/15 bg-white px-3 text-sm font-bold text-[#131E5C] outline-none transition hover:bg-slate-50 focus:ring-4 focus:ring-[#131E5C]/10">
                                    {buroOptions.map((s) => (<option key={s} value={s}>
                            ...
                            <div>
                                ...
                                <select value={filters.formaPago} onChange={(e) => updateFilter("formaPago", e.target.value)} className="h-10 w-full rounded-xl border border-[#131E5C]/15 bg-white px-3 text-sm font-bold text-[#131E5C] outline-none transition hover:bg-slate-50 focus:ring-4 focus:ring-[#131E5C]/10">
                                    {formaPagoOptions.map((s) => (<option key={s} value={s}>
                                        {s === "Todos" ? "Todos" : valueOrDash(s)}
                                    </option>))}
                                </select>
                            </div>
        ...
        {viewMode === "tabla" && (<div className="min-w-0">
            ...
            <div className="flex-1 min-w-0">
                <div className="hidden overflow-hidden rounded-lg bg-white border border-black/10 shadow-sm lg:block">
                            ...
                            <thead className="border-b border-slate-200 bg-white">
                                <tr className="text-xs">
                                    {[
                                        ...
                                        {key ? (<button type="button" onClick={() => toggleSort(key)} className="inline-flex items-center gap-1 text-xs font-bold text-[#131E5C] hover:text-[#131E5C]/70">
                                            {label}
                                        ...
                                        </button>) : (<span className="text-xs font-bold text-[#131E5C]">{label}</span>)}
                                    </th>))}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-black/[0.06]">
                                {loadingCases
                                    ...
                                    : paginatedRows.map((row) => {
                                        ...
                                        return (<tr key={row.id_exp} onDoubleClick={() => openEdit(row)} onContextMenu={(e) => onRowContextMenu(e, row)} onClick={() => setHighlightedRow(row)} className={cls("cursor-pointer hover:bg-[#131E5C]/[0.03] transition-colors", highlightedRow?.id_exp === row.id_exp ? "bg-[#131E5C]/[0.05]" : "")}>
                    ...
                    {/* Paginación */}
                    {sorted.length > 0 && (<div className="flex items-center justify-between px-4 py-3 border-t border-black/[0.06] bg-white">
                        ...
                        <div className="flex gap-1">
                            ...
                            ].map(({ label, action, disabled }) => (<button key={label} type="button" onClick={action} disabled={disabled} className="h-8 w-8 text-sm font-semibold rounded-lg border border-[#131E5C]/20 text-[#131E5C] disabled:opacity-40 hover:bg-slate-50">
                                {label}
                            </button>))}
                        </div>
                    </div>)}
